...
class Renderer(object):
    ...
    To implement concrete renderers, override ``api`` property and ``render``
    function.
    ...
    def convert(cls, messages):
        ...
        Initially this method is an identity function, returns received
        messages.
        Response of this function will be used in ``render`` function.

        When you want your project to respond to multiple APIs, you will make
        view functions to
        return common intermediate objects. However, these objects cannot be
        sent to API without
        converting to ``dict``(or namedtuple in ``requests`` library).
        In these cases, you can inherit pre-defined renderers,
        e.g. FacebookRenderer, and override ``convert`` to conform
    ...
    def render(self, messages, receiver_id):
        ...
        When implementing render, do not forget to call ``convert``
...
class NetworkRenderer(Renderer):
    ...
    def json_bodies(self, messages, receiver_id):
        ...
        Returned type may be list of any types that can be serialized by
        requests.
    ...
    def render(self, messages, receiver_id):
        ...
        for response in responses:
            if not response.ok:
                logging.warn('Failed status {}: {}'.format(response.status_code,
                                                           response.text))
